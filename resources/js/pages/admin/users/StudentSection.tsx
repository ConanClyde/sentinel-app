...
export function StudentSection({ users, activeRole, onView, onEdit, onDelete, pagination, roleSlug }: SectionProps) {
    return (
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead className="font-semibold">Program / College</TableHead>
                        ...
                        {users.length === 0 ? (
                            ...
                            users.map((user: any) => (
                                <TableRow key={user.id} className="hover:bg-muted/50 transition-colors">
                                    ...
                                    <TableCell className="font-mono text-sm">{user.student_id || 'N/A'}</TableCell>
                                    <TableCell>
                                        <div className="flex flex-col">
                                            <span className="text-sm font-medium">{user.program?.name || 'N/A'}</span>
                                            <span className="text-xs text-muted-foreground">{user.college?.name || 'N/A'}</span>
                                        </div>
                                    </TableCell>
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex justify-between items-start">
                                        <div className="flex flex-col min-w-0">
                                            ...
                                            <span className="text-xs font-mono text-muted-foreground">{user.student_id || 'NO ID'}</span>
                                        </div>
                                        <DropdownMenu modal={false}>
                                            <DropdownMenuTrigger asChild>
                                    ...
                                    <div className="text-sm text-muted-foreground flex items-center gap-1.5 mt-1 border-b pb-3 mb-3">
                                        <Mail className="h-3.5 w-3.5 shrink-0" />
                                        <span className="truncate">{user.email}</span>
                                    </div>
                                    <div className="space-y-1">
                                        <div className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">Program & College</div>
                                        <div className="text-xs font-medium">{user.program?.name || 'N/A'}</div>
                                        <div className="text-[10px] text-muted-foreground">{user.college?.name || 'N/A'}</div>
                                    </div>
