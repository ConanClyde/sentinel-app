...
import { CheckCircle2, Eye, Mail, MoreHorizontal, Pencil, Trash2 } from 'lucide-react';
...
export function SecuritySection({ users, activeRole, onView, onEdit, onDelete, pagination, roleSlug }: SectionProps) {
    return (
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead className="font-semibold">Staff ID / Security ID</TableHead>
                        ...
                        {users.length === 0 ? (
                            ...
                            users.map((user: any) => (
                                <TableRow key={user.id} className="hover:bg-muted/50 transition-colors">
                                    ...
                                    <TableCell>
                                        <span className="font-mono text-sm">{user.staff_id || '—'}</span>
                                    </TableCell>
                                        ...
                                        <DropdownMenu modal={false}>
                                            ...
                                            <DropdownMenuContent align="end" className="w-48">
                                                ...
                                                {onEdit && (<DropdownMenuItem onSelect={() => onEdit && onEdit(user)}>
                                                    <Pencil className="mr-2 h-4 w-4" /> Edit Personnel
                                                </DropdownMenuItem>)}
                                                <DropdownMenuItem onSelect={() => onDelete && onDelete(user.id)} className="text-destructive">
                                                    <Trash2 className="mr-2 h-4 w-4" /> Revoke Credentials
                                                </DropdownMenuItem>
                                            </DropdownMenuContent>
                                        </DropdownMenu>
                                    </TableCell>
                                ...
                                <div className="min-w-0 flex-1">
                                    <div className="flex items-start justify-between">
                                        <div className="flex min-w-0 flex-col">
                                            <span className="text-foreground truncate font-bold">{user.name}</span>
                                            <div className="mt-0.5 flex items-center gap-1 text-[10px] font-bold tracking-tighter text-emerald-600 uppercase">
                                    ...
                                    <div className="text-muted-foreground mt-2 flex items-center gap-1.5 border-t pt-2 text-sm">
                                        <Mail className="h-3.5 w-3.5 shrink-0" />
                                        <span className="truncate">{user.email}</span>
                                    </div>
                                    <div className="mt-3 flex items-center gap-2">
                                        <span className="font-mono text-xs">{user.staff_id || '—'}</span>
                                    </div>
