...
export function StakeholderRegistration({ role: roleProp, stakeholderTypes, vehicleTypes }: StakeholderRegistrationProps) {
    const role = roleProp || 'stakeholder';
    const { data, setData, post, processing } = useForm({
        ...
        student_school_id_image: null as File | null,
    ...
    const nextStep = () => {
        ...
        if (currentStep === 2) {
            if (!data.stakeholder_type) {
                toast.error('Please select stakeholder type');
                return;
            }
            if (isGuardian && !data.student_school_id_image) {
                toast.error('Please upload student school ID photo');
                return;
            }
        }
    ...
    const isGuardian = stakeholderTypes?.find(t => String(t.id) === data.stakeholder_type)?.name === 'Guardian';
    ...
    const handleCameraCapture = (file: File) => {
        ...
        } else if (cameraField === 'student_school_id_image') {
            setData('student_school_id_image', file);
        }
        setCameraOpen(false);
        setCameraField(null);
    };
    ...
    const clearFile = (field: string) => {
        ...
        } else if (field === 'student_school_id_image') {
            setData('student_school_id_image', null);
        }
    };
    ...
    const renderStep2 = () => (
        <>
            <h3 className="font-semibold text-lg">Stakeholder Details</h3>
            <div className="space-y-2">
                <Label>Stakeholder Type</Label>
                <Select value={data.stakeholder_type || undefined} onValueChange={(value) => setData('stakeholder_type', value)}>
                    <SelectTrigger className="h-10">
                        <SelectValue placeholder="Select type" />
                    </SelectTrigger>
                    <SelectContent className="w-[var(--radix-select-trigger-width)]">
                        {stakeholderTypes?.map((type) => (
                            <SelectItem key={type.id} value={String(type.id)} className="pl-3">{type.name}</SelectItem>
                        ))}
                    </SelectContent>
                </Select>
            </div>
            {isGuardian && (
                <div className="flex flex-col gap-2">
                    ...
                    <CameraPhotoButton
                        ...
                        hasPhoto={!!data.student_school_id_image}
                        capturedFile={data.student_school_id_image}
                        onCapture={() => openCamera('student_school_id_image')}
            ...
            <CameraCaptureDialog
                ...
                title={cameraField === 'student_school_id_image' ? "Scan Student's School ID" : "Scan Driver's License"}
            />
        </>
    );
        ...
        return (
            <div className="space-y-6 text-center">
                <div className="py-8">
                    ...
                    <p className="text-muted-foreground mb-6">The stakeholder has been registered successfully.</p>
            ...
            <div className="flex justify-between gap-4">
                ...
                {currentStep < totalSteps ? (
                    ...
                    <Button type="submit" disabled={processing}>
                        ...
                        {processing ? 'Creating...' : 'Register Stakeholder'}
                    </Button>
                )}
            </div>
        </form>
