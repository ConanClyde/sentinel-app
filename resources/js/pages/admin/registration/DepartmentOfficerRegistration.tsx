...
interface DepartmentOfficerRegistrationProps {
    role?: string;
    departments?: Department[];
}

export function DepartmentOfficerRegistration({ departments }: DepartmentOfficerRegistrationProps) {
    const role = 'department-officer';
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!data.department_id) { toast.error('Please select a department'); return; }
        ...
        return (
            <div className="space-y-6 text-center">
                <div className="py-8">
                    ...
                    <p className="text-muted-foreground mb-6">The department officer has been registered successfully.</p>
                    <Button type="button" onClick={handleRegisterAgain}>Register Another Department Officer</Button>
                </div>
            </div>
        );
    }
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <h3 className="font-semibold text-lg">Department Officer Details</h3>
            <div className="space-y-2">
                <Label>Department</Label>
                <Select value={data.department_id || undefined} onValueChange={(value) => setData('department_id', value)}>
                    <SelectTrigger className="h-10"><SelectValue placeholder="Select department" /></SelectTrigger>
                    <SelectContent className="w-[var(--radix-select-trigger-width)]">
                        {departments?.map((dept) => (
                            <SelectItem key={dept.id} value={String(dept.id)} className="pl-3">{dept.name}</SelectItem>
                        ))}
                    </SelectContent>
                </Select>
            </div>
            ...
            <Button type="submit" className="w-full" disabled={processing}>
                ...
                {processing ? 'Creating...' : 'Register Department Officer'}
            </Button>
        </form>
    );
}
